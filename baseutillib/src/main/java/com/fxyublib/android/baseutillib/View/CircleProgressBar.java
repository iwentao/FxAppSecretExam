...
public class CircleProgressBar extends View {
    ...
    private static final int DEFAULT_LINE_COUNT = 180;//45;
    private static final float DEFAULT_LINE_WIDTH = 4; //4
    ...
    private void drawProgressText(Canvas canvas) {
        ...
        CharSequence progressSubText = null;
        float yMain = 0;
        float ySub = 0;
        ...
        if(mMainTitle != null && !TextUtils.isEmpty(mMainTitle)) {
            progressText = mMainTitle;
            yMain = mCenterY + mProgressTextRect.height() / 2;
        }

        if(mSubTitle != null && !TextUtils.isEmpty(mSubTitle)) {
            progressSubText = mSubTitle;
            yMain = mCenterY - mProgressTextRect.height() / 2;
            ySub = mCenterY + mProgressTextRect.height() / 2*3;
        }

        mProgressTextPaint.setColor(mProgressTextColor);
        mProgressTextPaint.setTextSize(mProgressTextSize);
        Typeface fontInner = mTextFont;
        if(fontInner == null) fontInner = Typeface.SANS_SERIF;
        Typeface font = Typeface.create(fontInner, Typeface.BOLD);
        mProgressTextPaint.setTypeface( font );
        mProgressTextPaint.getTextBounds(String.valueOf(progressText), 0, progressText.length(), mProgressTextRect);
        canvas.drawText(progressText, 0, progressText.length(), mCenterX, yMain, mProgressTextPaint);

        //
        if(progressSubText != null) {
            mProgressTextPaint.setTextSize(mProgressTextSize-10);
            font = Typeface.create(fontInner, Typeface.NORMAL);
            mProgressTextPaint.setTypeface( font );
            mProgressTextPaint.getTextBounds(String.valueOf(progressSubText), 0, progressSubText.length(), mProgressTextRect);
            canvas.drawText(progressSubText, 0, progressSubText.length(), mCenterX, ySub, mProgressTextPaint);
        }
    }
    ...
    private void drawLineProgress(Canvas canvas) {
       ...
       // add by yub
        canvas.drawArc(mProgressRectF, 0.0f, MAX_DEGREE * mMax / mMax, false, mProgressPaint);
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        int offset = Math.abs(w-h)/2;
        mBoundsRectF.left = getPaddingLeft() + offset;
        mBoundsRectF.top = getPaddingTop();
        mBoundsRectF.right = radius - getPaddingRight() + offset;
        mBoundsRectF.bottom = radius - getPaddingBottom();
        ...
        //modify by yub, +20
        mProgressRectF.inset(mProgressStrokeWidth / 2 +10, mProgressStrokeWidth / 2+10);
    }
